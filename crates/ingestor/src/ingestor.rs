...
impl OtlpIngestor {
    /// Beging building a new ingestor up for configuration before finally calling [`Self::serve_grpc`]
    pub fn builder() -> Self {
        Self
    }

    /// Consume the ingestor and beging serving.
    pub async fn serve_grpc(self, addr: impl Into<SocketAddr>) -> Result<(), TonicError> {
